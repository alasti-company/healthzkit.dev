...
The **`@healthzkit/rabbitmq`** package ships ready-made [`HealthAdapter`](./checks-and-adapters.md) implementations for common AMQP clients. Each adapter measures round-trip latency, returns **`ok`** with `metadata.latencyMs` (and any extra fields from an optional hook), or **`fail`** with the caught error.
...
Combine with **`schedule`** on the check if you want cached readiness results instead of opening channels on every probe (see [Scheduling](./scheduling.md)).
