...
The **`@healthzkit/redis`** package ships ready-made [`HealthAdapter`](./checks-and-adapters.md) implementations for common Redis clients. Each adapter measures round-trip latency, returns **`ok`** with `metadata.latencyMs` (and any extra fields from an optional hook), or **`fail`** with the caught error.
...
Combine with **`schedule`** on the check if you want cached readiness results instead of hitting Redis on every probe (see [Scheduling](./scheduling.md)).
