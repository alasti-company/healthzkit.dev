...
describe("src/shared.ts", () => {
  ...
  test("buildResult merges extra metadata", () => {
    const r = buildResult(1, { brokerCount: 3 });
    expect(r.metadata).toEqual({ latencyMs: 1, brokerCount: 3 });
  });
  ...
  test("buildErrorResult wraps Error instances", () => {
    const r = buildErrorResult(new Error("broker unavailable"));
    expect(r.status).toBe("fail");
    expect(r.error).toBeInstanceOf(Error);
    expect((r.error as Error).message).toBe("broker unavailable");
  });
