...
describe("src/shared.ts", () => {
  ...
  test("buildResult merges extra metadata after latencyMs", () => {
    const r = buildResult(3, { index: "logs" });
    expect(r.metadata).toEqual({ latencyMs: 3, index: "logs" });
  });

  test("buildErrorResult wraps Error instances", () => {
    const r = buildErrorResult(new Error("cluster unavailable"));
    expect(r.status).toBe("fail");
    expect(r.error).toBeInstanceOf(Error);
    expect((r.error as Error).message).toBe("cluster unavailable");
  });
