...
MySQL **healthzkit** `HealthAdapter` helpers for **[`mysql`](https://github.com/mysqljs/mysql)** (legacy callback driver) and **[`mysql2`](https://github.com/sidorares/node-mysql2)** (promise API). Successful checks run a lightweight SQL probe (default **`SELECT 1`**) and return `ok` with `metadata.latencyMs` plus any fields from an optional `metadata` hook; failures return `fail` with the caught error.

## Install

```bash
npm install @healthzkit/mysql healthzkit
# plus one of:
npm install mysql
npm install mysql2
```
...
| Import                     | Exports                                    |
| -------------------------- | ------------------------------------------ |
| `@healthzkit/mysql`        | `mysqlAdapter`, `mysql2Adapter`, and types |
...
Both factories accept `BaseMysqlOptions`:
...
The adapter lazily imports `mysql`, parses a `mysql://` or `mysql2://` URL, and creates a shared internal **`Pool`**. Concurrent checks reuse that pool.
...
Pass **`client`** as a **`Pool`** or **`Connection`** from your app—the same acquire → query → release behavior applies.
...
For busy databases, pair these adapters with a **`schedule`** on the check so readiness reads cached results instead of running SQL on every probe. See the **Scheduling** section in the `healthzkit` README.
...
```bash
vp install
vp test --filter @healthzkit/mysql
vp pack --filter @healthzkit/mysql
```
